...
func (r *Repo) createDataset(ctx context.Context, tx *sql.Tx, ds *models.NewDataset, user *auth.User)(*models.Dataset, error){
	...
	return datasetFromSQL(created), nil
}
...
func (r *Repo) CreateDatasets(ctx context.Context, datasets []*models.NewDataset, user *auth.User) ([]*models.Dataset, error) {
	...
	for _, ds := range datasets {
		created, err:= r.createDataset(ctx, tx, ds, user)
		if err!= nil{
			if err := tx.Rollback(); err != nil {
				r.log.WithError(err).Error("Rolling back creation of dataset transaction")
				return nil, err
			}
			return nil, err
		}
		createdDatasets = append(createdDatasets, created)
	}
	if err := tx.Commit(); err != nil {
		r.log.WithError(err).Error("Commit creation of dataset transaction")
		return nil, err
	}
